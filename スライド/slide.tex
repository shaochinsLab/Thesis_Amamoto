  ...
  \begin{frame}{目次}
    ...
    \begin{frame}{研究背景と目的：スケジューリング問題との対応}
      ...
      \begin{itemize}
        ...
        \item 待ち時間を短縮させるための生産計画を立てることは重要な課題である．
      \end{itemize}
    ...
    \begin{frame}{諸定義：最大待ち時間最小化問題}
      以下，最大待ち時間最小化問題を {\it \alert{S}equencing to minimize maximum \alert{W}aiting \alert{T}ime} の頭文字をとって，\alert{SWT} と表記する．
    ...
    \begin{frame}{諸定義：最大待ち時間最小化問題}
      ...
      \begin{block}{目的関数：最大待ち時間 $\textcolor{white}{W_{\max}}$}
        \begin{itemize}
          \item ジョブの処理開始可能時刻と処理開始時刻の差が最大のもの．
          \begin{itemize}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \section{従来研究}
    \begin{frame}{従来研究：既存のスケジューリング問題との対応}
      \begin{block}{処理開始可能時刻付き最大遅れ時間最小化問題 (SRTD) との対応}
        SRTD は，ジョブの納期から処理が完了するまでの遅れ時間が最大のものの最小化を目的とするスケジューリング問題である．
        \vspace{-4mm}
        \begin{figure}
          \centering
          \includegraphics[width=10cm]{figure/SRTD1.pdf}
          \vspace{-2mm}
        \end{figure}
        納期を処理開始可能時刻と処理時間の和と設定したとき，
        \vspace{-4mm}
        \begin{figure}
          \centering
          \includegraphics[width=10cm]{figure/SRTD2.pdf}
          \vspace{-3mm}
        \end{figure}
        SRTD における遅れ時間は\alert{待ち時間}として捉えることができる．
      \end{block}
    \end{frame}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \section{研究成果}
    \begin{frame}{諸定義：待ち時間制約付きスケジューリング問題}
      待ち時間制約付きスケジューリング問題は，待ち時間 $w$ 以下となるスケジュールが存在するか，という決定問題である．つまり，判定結果は，
      \begin{itemize}
        \item $w$ 以下となるスケジュールが存在する場合 Yes，
        \item $w$ 以下となるスケジュールが存在しない場合 No，
      \end{itemize}
      となる．
    \end{frame}
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      ...
      \begin{alertblock}{成果 1 のまとめ}
        本研究では，SWT が NP 困難であることを明らかにした．
        NP 困難な問題に対する効率的な解法は発見されていない．
        そのため，最適解を求めるために，解の全列挙の基づいた解法が必要である．
      \end{alertblock}
    \end{frame}
    ...
    \begin{frame}{研究成果：ヒューリスティックの提案}
      \begin{alertblock}{成果 3}
        同一並列機械モデルにおける SWT に対して，貪欲解法に基づいたヒューリスティックの開発と解法に対する実験的評価を行った．
      \end{alertblock}
      ...
      \end{block}
      \begin{itemize}
        \item ここで，$B = 0$，$A \neq 0$とき，競合比は $\infty$ となる．
        \item $B = 0$ のとき，提案したヒューリスティックは，常に $A = 0$ となる．
      \end{itemize}
    \end{frame}

    \begin{frame}{研究成果：ヒューリスティックの精度について}
      \begin{block}{JIT ジョブ荷重和最大化問題 (SJIT) との対応}
        SJIT において，すべてのジョブを JIT で処理可能なとき，各ジョブは処理開始可能時刻と納期の区間ちょうどで処理される．
        \begin{figure}[h]
          \vspace{-3mm}
          \centering
          ...
          \vspace{-3mm}
        \end{figure}
        すべてのジョブが JIT で処理可能なとき，待ち時間 $0$ で処理可能である．
        \vspace{-5mm}
        \begin{itemize}
          \setlength{\leftskip}{-4mm}
          \item SJIT において，すべてのジョブを JIT で処理できるとき，処理開始可能時刻の昇順で処理することで最適解が求まる．[Cepek \& Sung(2004)]
        \end{itemize}
      \end{block}
      \vspace{-2mm}
      \begin{itemize}
        \item SWT  において，すべてのジョブの待ち時間が $0$ のとき，処理開始可能時刻の昇順で処理することで最適解が求まる．
      \end{itemize}
      \vspace{-2mm}
    ...
    \begin{frame}{付録：ヒューリスティックの精度について}
    ...
    \begin{frame}{付録：ヒューリスティックの精度の保証の証明}
      \begin{block}{精度の保証の証明}
        ...
        したがって，最適解から得られる最大待ち時間が $0$ となるインスタンスに対して，処理開始可能時刻の昇順で，処理開始可能な機械に割り当てることで最適なスケジュールを作ることができる．
