  ...
  \begin{frame}{目次}
    ...
    \begin{frame}{諸定義：最大待ち時間最小化問題}
      ...
      \begin{block}{目的関数：最大待ち時間 $\textcolor{white}{W_{\max}}$}
        \begin{itemize}
          \item ジョブの処理開始可能時刻と処理開始時刻の差が最大値．
          \begin{itemize}
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \section{研究成果}
    \subsection{最大待ち時間最小化問題の計算複雑さの証明}
    \begin{frame}{既存のスケジューリング問題との対応}
      処理開始可能時刻付き最大遅れ時間最小化問題 (SRTD) は，ジョブの納期から処理が完了するまでの遅れ時間が最大値の最小化を目的とするスケジューリング問題である．
    ...
    \begin{frame}{待ち時間制約付きスケジューリング問題}
      SWT の計算複雑さを明らかにするために，NP 完全であることが知られている {\sc 3-SATISFIABILITY (3-SAT)} から\alert{多項式還元}を行う．
      \begin{tabular}{cc}
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      ...
      \end{alertblock}
      \begin{block}{還元のポイント}
        待ち時間制約付きスケジューリング問題における最大待ち時間は，
        \begin{itemize}
          \item {\sc 3-SAT} の判定結果が \alert{Yes} のとき， \alert{2 以下}，
          \item {\sc 3-SAT} の判定結果が \alert{No} のとき，\alert{3 以上}となる ．
        \end{itemize}
      \end{block}
