  ...
  \begin{frame}{目次}
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      ...
      \begin{block}{多項式時間還元のポイント}
        \begin{itemize}
          \item {\sc 3-SAT} のインスタンスにおける\alert{変数に対応する機械}
          \item {\sc 3-SAT} のインスタンスにおける\alert{節に対応する機械}
          \item $1$ に割り当てられたリテラルに対応するジョブは次のジョブを\alert{押し出さない}，$0$ に割り当てられたジョブは次のジョブを \alert{$1$ 押し出す}．
        \end{itemize}
      \end{block}
    \end{frame}
