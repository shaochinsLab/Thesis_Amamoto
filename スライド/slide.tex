...
\begin{frame}{目次}
  ...
  \begin{frame}{目次}
    ...
    \end{frame} %目次スライド
    \section{研究背景}
    \begin{frame}{研究背景：受注生産方式における生産計画の流れ}
    ...
    \begin{frame}{問題の分析：既存のスケジューリング問題との対応}
      ...
      \begin{block}{SRTD と SJIT の計算複雑さ}
        \begin{itemize}
          \item SJIT は，{\sc 3-SAT} からの還元により，無関連並列機械モデルにおいて機械数が入力の一部の場合，\alert{強 NP 困難}である．[Sung \& Vlach (2005)]
          \item SRTD は，{\sc 3-PARTITION} からの還元により，単一機械モデルにおいて，\alert{強 NP  困難}である．[Garey \& Johnson (1990)]
        \end{itemize}
      \end{block}
    \end{frame}
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %     \begin{itemize}
    %       \item NP 完全である {\sc 3-SAT} から無関連並列機械モデルにおける待ち時間制約付きスケジューリング問題が\alert{多項式還元}できることを示した．
    %       \item 待ち時間制約付きスケジューリング問題の NP 完全性を明らかにすることで，SWT が \alert{NP 困難}であることを示した．
    %     \end{itemize}
    %   \end{alertblock}
    % \end{frame}
    %
    % \begin{frame}{研究成果：多項式還元とは}
    %
    %   \begin{tabular}{cc}
    ...
    %           「$A$ は $B$ に多項式還元可能」という．
    %         \end{center}
    %         $B$ を解くアルゴリズムを用いて，$A$ も解くことができるため，$B$ は $A$ と同等か，より難しいことがわかる．
    %       \end{block}
    %     \end{minipage}
    ...
    %     \vspace{5mm}
    %   \end{tabular}
    %   本研究では，$A$ と $B$ を以下の問題として多項式還元を行った．
    %   \begin{description}
    %     \item[問題 $A$ :] ${\textcolor{red}{\mbox {\sc 3-SAT}}}$
    %     \item[問題 $B$ :] {\textcolor{red}{待ち時間制約付きスケジューリング問題}}
    %   \end{description}
    % \end{frame}
    ...
    % \begin{frame}{研究成果：問題の計算複雑さ}
    %   各 $i \in \{ 1,\ldots,n\}$ において，$\alpha_i \ge 3$，$\beta_i \ge 3$ のとき，次のように待ち時間制約付きスケジューリング問題のインスタンスを設定する．
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      ...
      \begin{block}{変換した待ち時間制約付きスケジューリング問題のインスタンス}
        ...
        \begin{itemize}
          \item $n$ は $X$ の要素数．
          各変数に対応する機械を 1 台ずつ用意する．
          \item $\lambda$ は $H$ の要素数．
          各節に対応する機械を 1 台ずつ用意する．
        \end{itemize}
      \end{block}
      \begin{itemize}
        \item $f(x_i) = 1$ のとき，
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      \begin{block}{各機械 ${\textcolor{white}{\ell \in \{n + 1,\ldots,n + \lambda\}}}$ に対応するジョブのスケジュール}
        機械 $\ell \in \{n + 1,\ldots,n + \lambda\}$ に割り当てるジョブは，節 $h$ の要素であるリテラルに対応するジョブ集合の要素 1 つ．
        例えば，$h_{\ell - n} = \{x_i, x_j, x_k\}$ のとき，
        \begin{description}
          \setlength{\leftskip}{-12mm}
          \item[$S$：]
          $f(x_i) = 1, f(x_j) = 0, f(x_k) = 0$ のとき，機械 $\ell$ に割り当てるジョブは，$J_i \in \mathcal{J}^t(i)$，$J_j \in \mathcal{J}^f(j)$，$J_k \in \mathcal{J}^f(k)$ とダミージョブ $J_d$．
          \item[$S'$：]
          $f(x_i) = 0, f(x_j) = 0, f(x_k) = 0$ のとき，機械 $\ell$ に割り当てるジョブは，$J_i \in \mathcal{J}^f(i)$，$J_j \in \mathcal{J}^f(j)$，$J_k \in \mathcal{J}^f(k)$ とダミージョブ $J_d$．
        \end{description}
        \vspace{-5mm}
        \begin{figure}[h]
          \centering
    ...
    %       \end{minipage}
    %     \end{tabular}
    %     ${\displaystyle \frac{\text{最適でないスケジュールの最大待ち時間}}{\text{厳密解法から得られた解の最大待ち時間}} = \infty}$
    %   \end{exampleblock}
    %   \begin{alertblock}{ヒューリスティックの実験的評価のまとめ}
    %     本研究で提案したヒューリスティックは，上記の比が最大 $24$ となり，どのようなインスタンスに対しても，上記の比が定数であることが保証されている．
    %   \end{alertblock}
    % \end{frame}

    \begin{frame}{研究成果：ヒューリスティックの精度について}
      \begin{block}{ヒューリスティックの精度}
        本研究では，ヒューリスティックから得られた解における最大待ち時間と厳密解法から得られた解における最大待ち時間の比に着目し実験を行った．ただし，以下の式でヒューリスティックの最悪計算量を表す．
        $${\displaystyle \frac{\text{ヒューリスティックから得られた解における最大待ち時間}\cdots A}{\text{厳密解法から得られた解における最大待ち時間}\cdots B}}$$
        ここで，$B = 0$，$A \neq 0$とき，上記の比は $\infty$ となる．
        しかし，本研究で提案したヒューリスティックは，$B = 0$ のとき，$A = 0$ となる．つまり，上記の比が常に定数である．
      \end{block}
      \begin{alertblock}{ヒューリスティックの実験的評価のまとめ}
