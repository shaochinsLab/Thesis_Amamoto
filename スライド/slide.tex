...
\begin{frame}{目次}
  ...
  \begin{frame}{目次}
      ...
      \begin{block}{機械モデル}
        \begin{itemize}
          ...
          \begin{itemize}
            \item すべての機械の性能が等しい．
            \begin{itemize}
              \item 複数の製造工場で製造し，各製造工場は同じ機械を使っている．
            \end{itemize}
          \end{itemize}
          ...
          \begin{itemize}
            \item 機械ごとに処理速度が異なる．
            \begin{itemize}
              \item 複数の製造工場で製造し，製造工場ごとに機械の性能が異なる．
            \end{itemize}
          \end{itemize}
          ...
          \begin{itemize}
            ...
            \begin{itemize}
              \item 複数の製造工場で製造し，製造工場ごとに，異なるメーカーの機械を導入しており，製造する製品によって製造期間が異なる．
            \end{itemize}
          \end{itemize}
        \end{itemize}
      \end{block}
    ...
    \begin{frame}{多項式時間還元の流れ}
      SWT の NP 完全性を示すために，NP 完全であることが知られている {\sc 3-SATISFIABILITY (3-SAT)} から\alert{多項式時間還元}を行う．
      \begin{tabular}{cc}
    ...
    \begin{frame}{研究成果：問題の計算複雑さ}
      ...
      \begin{block}{多項式時間還元のポイント}
        \begin{itemize}
          \item {\sc 3-SAT} のインスタンスにおける変数に対応する機械
          \item {\sc 3-SAT} のインスタンスにおける節に対応する機械
          \item $1$ に割り当てられたリテラルに対応するジョブは次のジョブを押し出さない，$0$ に割り当てられたジョブは次のジョブを押し出す．
        \end{itemize}
      \end{block}
    \end{frame}
