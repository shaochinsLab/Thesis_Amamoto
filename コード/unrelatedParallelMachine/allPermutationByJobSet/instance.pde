class Instance {
	...
	for(int i = 0 ; i < jobs.size() ; i++){
	    ...
	    completionTime[machineNumber] = startTime + jobs.get(i).processingTime[machineNumber] ;
	...
	if(bestValue == 0)
	    return ;
	Assignment as = new Assignment(jobs.size(), numOfMachines) ;
	int[] order = new int[numOfMachines] ;
	do{
	    if(firstEvaluation(as, order))
		getSingleMachineInstance(as, order) ;
	}
	while(as.increment()) ;
    }
    boolean firstEvaluation(Assignment as, int[] order){
	for(int i = 0 ; i < order.length ; i++)
	    order[i] = i ;
	int[] minCost = new int[numOfMachines] ;
	for(int i = 0 ; i < as.body.length ; i++)
	    jobs.get(i).machineNumber = as.body[i] ;
	for(int i = 0 ; i < as.upperLimit + 1 ; i++){
	    JobSet jobSet = new JobSet() ;
	    for(int j = 0 ; j < jobs.size() ; j++){
		if(jobs.get(j).machineNumber == i)
		    jobSet.add(jobs.get(j)) ;
	    }
	    SingleMachineInstance si = new SingleMachineInstance(jobSet, bestValue, i) ;
    ...
    void getSingleMachineInstance(Assignment as, int[] order){
	for(int i = 0 ; i < order.length ; i++){
	    JobSet jobSet = new JobSet() ;
	    for(int j = 0 ; j < jobs.size() ; j++){
	    ...
	    SingleMachineInstance si = new SingleMachineInstance(jobSet, bestValue, order[i]) ;
	...
	updateBestPermutation(as) ;
    }
    void updateBestPermutation(Assignment as){
	int partitionBestValue = getPartitionBestValue() ;
	if(partitionBestValue < bestValue){
	    bestPartition = (int[])as.body.clone() ;
