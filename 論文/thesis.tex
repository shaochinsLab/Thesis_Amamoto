...
ここで，表記の導入を行う．ジョブの配列を $js$，ジョブ数を $n$，機械数を $m$，整数 $W$（初期解）と表記する．
  ...
  \begin{description}
    ...
    \begin{description}
      ...
      \begin{description}
        ...
        り返す．ここで，$numOfJobs$ を定義し，初期値を 0 とする．
        \begin{description}
          \item[Step 1.1 ] 以下の処理を 整数 $j$ が 0 から $n$ まで
          繰り返す．
          \begin{description}
            \item[ 1.1.1 ] $j = i$ のとき，$list$ に $js$ の $j$ 番目のジョブを
            加える．ここで，$numOfJobs$ をプラス 1 していく．
          \end{description}
          \item[Step 1.2 ]  $(list,numOfJobs,i,0,0,0)$ を入力と
...
\begin{quote}
  \begin{description}
    ...
    \item[入力：]  双方リスト $list$，整数 $numOfJobs$，$machineNumber$，$i$，$completionTime$，$waitingTime$ の 6 項組 $(list,numOfJobs,machineNumber,\\completionTime,waitingTime)$
    \item[出力：] $waitingTime$ が最も小さいスケジュールとなるジョブの配列\\ $permutation$
    \begin{description}
      \item[] $i = numOfJobs$ のとき，
      \begin{description}
        \item[Step 1 ] $solution[machineNumber] : = permutation$ とする．
      \end{description}
      \item[] $i \neq numOfJobs$ のとき，
      \begin{description}
        \item[Step 2 ] 最初に参照するジョブを $list$ の $head$ と
        する．また，以下の処理を参照しているジョブの前方リ
        ンクに対応するジョブが $head$ になるまで繰り返す．
        \begin{description}
          \item[Step 2.1] 参照するジョブを現在参照しているジョ
          ブの前方リンクに対応するジョブとし，$job$ と表記す
          る．
          \item[Step 2.2 ] $permutation[i] := job$ とする．整数 $s$，$c$，$w$ を次のように定義する．また，参照しているジョブの処理開始可能時刻を $r$，処理時
          間を $p$ と表記する．
          $s = \max\{completionTime, r\}$，$c = s + p$，$w =
          \max\{waitingTime,s - r\}$
          \item[Step 2.3 ] $W \le w$ のとき，処理を中断する．
          \item[Step 2.4 ] $list$ から $job$ を取り
          除く．
          \item[Step 2.5 ] $list$ に対して，$(list,c)$ を入力
          として，{\sc evaluation} を適用する．
          \begin{description}
            \item[Step 2.5.1 ] {\sc evaluation} の出力が $true$ のと
            き，\\$(list,p,numOfJobs,machineNumber,i +
            1,c,w)$ を入力として {\sc allPermutation} を実
            行する．
          \end{description}
          \item[Step 2.6 ] $job$ を $list$ の取り除いた場所に
          戻す．
        \end{description}
      \end{description}
    \end{description}
