...
次に，決定問題の 1 つである {\sc 3-SATISFIABILITY (3-SAT)} と {\sc 3-PARTITION} を紹介する．
...
{\sc 3-SAT} と {\sc 3-PARTITION} の計算複雑さは NP 完全であることが Garey and Johnson \cite{3SAT} により証明されている．
...
本研究では，{\sc 3-SAT} からの多項式時間還元により 無関連並列機械モデルにおいて機械数が入力の一部の場合，待ち時間制約付きスケジューリング問題が NP 完全であることを示す．
つまり，{\sc 3-SAT} のインスタンスから待ち時間制約付きスケジューリング問題のインスタンスが多項式時間で構成可能であり，加えて， {\sc 3-SAT}  の判定結果が Yes ならば
...
この問題は　{\sc 3-SAT} からの還元により，強 NP 困難であることが Sung and Vlach \cite{SJIT} によって証明されている．以下では，JIT ジョブ荷重和最大化問題を定式化し，問題が NP 困難であることの証明を示す．
...
以下では，{\sc 3-SAT} からの還元により，JIT ジョブ荷重和最大化問題の計算複雑さの証明を紹介する．
...
この問題は {\sc 3-PARTITION} からの還元により，強 NP 困難であることが Garey and Johnson \cite{3SAT}によって，証明されている．
...
以下では，{\sc 3-PARTITION} からの還元により，処理開始可能時刻つき最大遅れ時間最小化問題の計算複雑さを証明を紹介する．

整数の集合 $S = \{a_1,\ldots,a_{3t}\}$ と 整数 $b$ の 2 項組 $(S,b)$ を {\sc 3-PARTITION} の
...
\begin{itemize}
  ...
  %このとき，スケジュールにおける各ジョブを順に $J_1,\ldots,J_n$，$C(0) = 0$ とすると，各ジョブは以下の条件を満たす．
  % \begin{displaymath}
...
ブール型変数の集合 $X =\{x_1, x_2,\ldots ,x_n\}$ と， $X$ 上の 3 つのリテラルからなる集合の集合 $H =\{h_1, h_2,\ldots ,h_{\lambda}\}$ の 2 項組，$(X,H)$ を任意の {\sc 3-SAT} 問題の入力とする．
...
ジョブ $\mathcal{J}^d = \{J^d(1),\ldots,J^d(n + \lambda)\}$ について，処理開始可能時刻関数 $r$，処理時間関数 $p$ を各 $i \in \{1,\ldots,n\}$ と 各 $k \in \{n + 1,\ldots,n + \lambda\}$ について以下のように設定する．

\begin{displaymath}
\left\{ \begin{array}{ll}
r(\mathcal{J}^d(i)) = 3(\mathcal{A} + \mathcal{B}) \\
r(\mathcal{J}^d(k)) = 3(\mathcal{A} + \mathcal{B}) \end{array} \right.
\end{displaymath}
\begin{displaymath}
...
\begin{proof}
  ...
  \mathcal{B}) + 5$ であるので，いずれのジョブも機械 $i$ に割り当てることができない．
  ...
  ここで，各 $k \in \{n + 1,\ldots,n + \lambda\}$ における $J^d(k)$ を各機械 $j$
  に 1 つずつ割り当てる．
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $3(\mathcal{A} + \mathcal{B}) + 3$ となる．ここで，$k \in
    \{n + 1,\ldots,n + \lambda\}$ におけるジョブ $J^d(k)$ をそれらの
    ジョブの後に割り当てるとき，ジョブ $J^d(k)$ に 3 の待ち時間が発生する．$|h| = 2,|h| = 1$ のとき，$B.1$ より，それぞれの場合において $J^d \in \mathcal{J}^d$ に 4 または 3 の待ち時間が発生する．（図 4.2）
  \end{itemize}
  ...
  \begin{itemize}
    ...
    $3(\mathcal{A} + \mathcal{B}) + 3$ となる．ここで，$k \in
    \{n + 1,\ldots,n + \lambda\}$ において，機械に割り当てられていないジョブ
    $J^d(k)$ をそれらのジョブの後に割り当てるとき，ジョブ
    $J^d(k)$ に 3 の待ち時間が発生する．
    \item 上記の 2 つのケースにおいて，$k \in \{n + 1,\ldots,n + \lambda\}$ において，機械に割り当てられていないジョブ $J^d(k)$ を機械 $i
    \in \{1,\ldots,n\}$ に割り当てた場合 $I_{(X,H)}$ より，ジョブ
    $J^d(j')$ に 5 の待ち時間が発生する．
  \end{itemize}
...
% \begin{description}
%   \item[尺度 5 :] 各インスタンスにおけるジョブの処理時間幅の増加に伴う競合比の変動．
%
%   図~\ref{5.3} より，処理時間の上限の増加に伴い，ヒューリスティックの質は悪くなっている．
%   しかし，処理時間の上限が 50 のとき，上限が 40 のときに比べ，ヒューリスティックの質は良くなっている．処理時間の上限が競合比に与える影響は，本研究では明らかにできなかった．
%   本研究で提案したヒューリスティックは，最適解における最大待ち時間との比率が無限大にならないことが保証されている．そのため，本研究で提案したヒューリステックは，最適解における最大待ち時間の定数倍に収まる．
%   実験の結果，ヒューリスティックによる解から得られた最大待ち時間と最適解における最大待ち時間の比率は 24 であった．また，ジョブ数が少ないときかつ機械数が少ないとき，もしくはジョブ数が多いときかつ機械数が多いときに競合比の値が小さくなる傾向があることがわかったが，その理由については示せていない．
%
%   この実験結果より，本研究で提案したヒューリスティックは，最適解における最大待ち時間の定数倍に収まるが，最大待ち時間最小化問題に対する有効的な解法であるとは言えない．
%
% \end{description}
...
%   \caption{尺度 5：各インスタンスにおけるジョブの処理時間幅の増加に伴う競合比の変動．表~\ref{A5} 参照．}
%   \label{5.3}
% \end{figure}
%
% \begin{description}
%   \item[尺度 6 :] 各インスタンスにおけるジョブの処理時間幅の増加に伴う計算時間の変動．
%
%   図~\ref{5.4}，\ref{5.5} より，機械数が 2 のとき，処理時間の上限の増加に伴い計算時間も増加していた．しかし，機械数 3 のとき，処理時間の上限の増加に伴い計算時間は減少していた．この結果は，機械数 2 と 3 の間だけでなく，表~\ref{A6} より，機械数 3 以降においても，機械数の増加に伴い計算時間は減少していた．本研究では，処理時間の上限と機械数の関係が，計算時間に与える影響については明らかにできなかった．
%
% \end{description}
%
% \begin{figure}[h]
%   \centering
%   \includegraphics[width = 14cm]{figure/ptTimeId1.pdf}
%   \caption{尺度 6：機械数 2 のとき，各インスタンスにおけるジョブの処理時間幅の増加に伴う計算時間の変動．表~\ref{A6} 参照．}
%   \label{5.4}
% \end{figure}
%
% \begin{figure}[h]
%   \centering
%   \includegraphics[width = 14cm]{figure/ptTimeId2.pdf}
%   \caption{尺度 6：機械数 3 のとき，各インスタンスにおけるジョブの処理時間幅の増加に伴う計算時間の変動．表~\ref{A6} 参照．}
%   \label{5.5}
% \end{figure}


% \begin{description}
%   \item[尺度 7 :] インスタンスサイズの増加に伴う，同一並列機械モデルと無関連並列機械モデルの競合比の比較．
%   %図~\ref{5.6} 参照．
% \end{description}

% \begin{figure}[h]
%   \centering
%   \includegraphics[width = 16cm]{figure/ptTimeId.pdf}
%   \caption{尺度 7：インスタンスサイズの増加に伴う，同一並列機械モデルと無関連並列機械モデルの競合比の比較．表~\ref{A5}，\ref{A7} 参照．}
%\label{5.6}
% \end{figure}

% \begin{description}
%   \item[尺度 8 :] インスタンスサイズの増加に伴う，同一並列機械モデルと無関連並列機械モデルの計算時間の比較．
%   %図~\ref{5.7} 参照．
% \end{description}
...
%   \centering
%   \includegraphics[width = 16cm]{figure/ptTimeId.pdf}
%   \caption{尺度 8：インスタンスサイズの増加に伴う，同一並列機械モデルと無関連並列機械モデルの計算時間の比較．表~\ref{A6}，\ref{A8} 参照．}
%\label{5.7}
% \end{figure}
...
\begin{description}
  ...
  JIT ジョブ荷重和最大化問題における {\sc 3-SAT} からの還元手法に着目し，待ち時間制約付きスケジューリング問題 の NP 完全性を示し，最大待ち時間最小化問題が NP 困難であることを示した．
...
%       14 & 3 & 269 & 1,049 & 1,535 & 1,728 & 1,796 \\ \hline \hline
%
%       & 2 & 33 & 76 & 163 & 548 & 1,696 \\ \cline{2-7}
%       15 & 3 & 1,151 & 4,116 & 5,811 & 6,744 & 7,649 \\ \hline \hline
%
%       & 2 & 89 & 188 & 595 & 3,099 & 11,308 \\ \cline{2-7}
%       16 & 3 & 4,737 & 14,861 & 19,667 & 21,928 & 24,588 \\ \hline \hline
%
%       & 2 & 251 & 545 & 3,148 & 20,600 & 83,626 \\ \cline{2-7}
%       17 & 3 & 18,057 & 51,805 & 63,698 & 67,722 & 82,634 \\ \hline \hline
%     \end{tabular}
