...
\begin{description}
  ...
  最大実行開始待ち時間最小化問題はどの機械モデルにおいても，計算複雑さが明らかではなかった．本研究では，\textsc{3-SAT} からの還元により，無関連並列機械モデルにおいて，機械数が入力の場合，NP 完全であることを証明した．
  \item[成果 2 :]
  対象とする環境における提案解法の有効性を示した．

  提案解法により出力された解から得られた目的関数の値を $W_h$，分枝限定法により出力された最適解より得られた目的関数の値を $W_{opt}$ とすると，異なるインスタンスに対して，$W_h/W_{opt}$ を求めることで，提案解法の評価を行った．$W_h/W_{opt}$ の値が 1 に近いほど，解法の質は良いと言える．本研究の実験によると，$\max\big\{W_h/W_{opt}\big\} = ?$ の結果を得た．

  \item[成果 3 :]
  部分問題に対する多項式アルゴリズムの概念を取り入れ，分析規模を拡大した．

  NP 困難な最適化問題は多項式時間で解を求めることができないと考えられている．
  そのため，最適解を求めるためには，実行可能解を全列挙し比較する必要がある．
  そこで，本研究では，分枝限定法に部分問題に対する多項式アルゴリズムの概念を取り入れ，探索する部分木を減らすことで，分析規模の拡大を可能にした．
\end{description}
...
\begin{description}
  ...
  \item[インスタンス : ] ジョブの集合 $\mathcal{J}$ を無関連機械の集合 $\mathcal{M}$と表記し，
  $J \in \mathcal{J}$ を $M \in \mathcal{M}$ で処理する．
  処理開始可能時刻関数 $r : \mathcal{J} \to \mathbb{N}$，処理
...
\begin{figure}[h]
  \centering
  \includegraphics[width = 11cm]{figure/reduction.pdf}
  \caption{多項式時間還元の流れ}
\end{figure}
...
\begin{itemize}
  ...
  実行開始待ち時間 $w$ の制約が最も強い場合，つまり，$w = 0$ のとき，各ジョブは処理開始可能時刻ちょうどで処理を開始しなければならない．このとき，最大実行開始待ち時間最小化問題は JIT ジョブスケジューリング問題として捉えることができる．

  \item \textbf{JIT ジョブ荷重和最大化問題との差分}

  JIT ジョブ荷重和最大化問題は，納期以前に処理を完了したジョブの荷重和
  最大化する問題である．対して，最大実行開始待ち時間最小化問題（決定問題）では，全てのジョブ
  ...
  処理開始可能時刻付き最大遅れ時間最小化問題では，処理開始可能時刻と納期の間で処理可能かという問題として捉えることができる．このとき，納期 := 処理開始可能時刻 + 処理時間 としたとき，納期により制限を加えた問題が，最大実行開始待ち時間最小化問題（決定問題）に対応する．
...
この問題は　\textsc{3-SAT} からの還元により，強 NP 困難であることが Sung, S. C., and Vlach(2005) \cite{JIT}
によって証明されている．以下では，JIT ジョブ荷重和最大化問題を定式化し，問題が NP 困難であることの証明を示す．
\subsection{定式化}
\begin{description}
...
\begin{proof}
  ...
  \begin{figure}[h]
    \centering
    \includegraphics[width = 15cm]{figure/SJIT1.png}
...
\begin{proof}
  ...
  \begin{figure}[h]
    \centering
    \includegraphics[width = 14cm]{figure/Lmax1.png}
    \caption{各 $i \in \{1,\ldots,t - 1\}$ におけるジョブ $J_i$ のス
    ケジュール}
  \end{figure}
...
式で最適解が求まることが Lawler [1973] \cite{EDD1}，Lageweg, Lenstra, and Rinnooy Kan [1976] \cite{EDD2} によって証明されている．つまり，処理開始可能時刻付き最大遅れ時間最小化問題の部分問題に対して，多項式アルゴリズムが存在する．処理開始可能時刻と処理時間，納期の制約を制限することで，多項式で最適解が求まる．部分問題に対する解法は以下の通り．
...
\begin{proof}
  ...
  \begin{figure}[h]
    \centering
    \includegraphics[width = 11cm]{figure/EDDrule.png}
  ...
  よって，$1\mid \mid L_{\max}$ 問題にEDD ルールを適用することで最適解を求めることができる．
\end{proof}
...
が明らかでない．本研究では，\textsc{3-SAT} からの還元によって，無関連並列機械モデルにおける最大実行開始待ち時間最小化問題（決定問題）が NP 完全であることを示した．
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    $I_{(X,H)}$ の $B.3$ より，それらのジョブは，スケジュールにおける次のジョブの処理開始可能時刻を 1 超える処理時間を持つ．$h_{j^{\prime}}$ に含
    ...
    ジョブの後に割り当てるとき，ジョブ $J^d_{j^{\prime}}$ に 3 の実行開始待ち時間が発生する．$|h| = 2,|h| = 1$ のとき，$B.1$ より，それぞれの場合において $J^d \in \mathcal{J}^d$ に 4 または 3 の待ち時間が発生する．（図 $G.7$）
  \end{itemize}
...
この章では，提案する解法の紹介と，その解法の評価をするために用いる分枝限定法について紹介する．また，提案解法により出力された解から得られた目的関数の値と分枝限定法により出力された最適解から得られた目的関数の値を比較し，提案解法より出力される解の質の分析を行った．
...
\noindent 以下の {\sc greedy Algorithm} が本研究の提案解法である．
...
上記 {\sc greedy Algorithm} は，処理開始可能時刻が最も早いジョブから順に，処理をしていない機械，または処理が一番早く終わった機械に割り当てる．
...
\begin{itemize}
  ...
  \item 要素数 $n$ の配列 $b_{\max}$ は 各 $1 \le i \le n$ に対して，$i$ 番目の要素は，それまでのジョブが割り当てられた機械の最大数を格納している．つまり，$b_{\max}[i] = \displaystyle \max_{1\le j \le i - 1}\bigg\{b[j]\bigg\}$．
\end{itemize}
...
\begin{quote}
  \begin{description}
    ...
    \begin{description}
      \item[Step 1.] $I$ を入力とし，{\sc greedy algorithm} を実行する．出力を $\mathcal{S}$ とする．このとき，$W = w(\mathcal{S})$ とする．
      ...
      \begin{description}
        ...
        \begin{description}
          ...
          \end{description}
          \item[Step 2.2.3.] 組 $(\mathcal{J}',s,r,p,w,C,0)$ を入力として，{\sc allPermutation} を実行する．
        \end{description}
      \end{description}
...
上記 \textsc{BrnchAndBound} では，\textsc{normalize} と \textsc{increment} により，分割 $\Pi$ の要素数 が 機械数 $m$ と等しくなるように，分割を生成している．その後 \textsc{allPermutation} により，各 $\pi \in \Pi$ に対して，順列の生成を行っている．
...
上記 {\sc normalize} は，{\sc increment} で生成した $b$ の各要素が $m$ 未満のとき，割り当てていない機械が存在するので，その分割 $b$ を $m$ となる要素を持つ $b$ に変換している．
Step 1.1. では，$n - i$ 番目の 1 つ前のジョブまでが割り当てられた機械番号の最大値が $m$ 以上のとき，すでに分割の要素数が $m$ であることから，変換せずに出力している．
...
\begin{quote}
  \begin{description}
    ...
    \item[出力 :] スケジュール $S$
    \begin{description}
      \item[Step 1.] 各 $i \in \{1,\ldots,|\mathcal{J}'|\}$ における $J'_i$ について，\\$p(J'_i) = \displaystyle \min_{j \in \{1,\ldots,|\mathcal{J}'|\}}\bigg\{p(J'_j)\bigg\}$ とする．
    ...
    \begin{description}
      ...
      \begin{description}
        ...
        \begin{description}
          ...
          \begin{description}
            \item[Step 3.2.1] $W \le w(S')$ のとき，$(\mathcal{J}',r,p,w,C,W,i + 1)$ を入力として {\sc allPermutation} を実行する．
          \end{description}
...
% allPermutation の説明
上記 {\sc allPermutation} は，再帰的に順列の生成を行っている．
...
\section{提案解法の評価と分析}
同一並列機械モデルにおける最大実行開始待ち時間最小化問題に対して，本研究で提案したヒューリスティックの実験的評価を行った．評価方法は，各インスタンスに対して，本研究で提案するアルゴリズムにより出力された解から得られた最大実行開始待ち時間と，分枝限定法により出力された最適解から得られた最大実行開始待ち時間を比較するものである．
...
\begin{description}
  \item[計算複雑さ ] \textsc{3-SAT} からの還元により，無関連並列機械モデルにおいて，機械数が入力の一部の場合，NP 完全であることを明らかにした．

  \item[解法 ] 貪欲アルゴリズムをもとに同一並列機械モデルにおける最大実行開始待ち時間最小化問題に対するヒューリスティックを開発し，実験的評価を行った．

  このヒューリスティックの実験的評価を行い，対象とする環境における有効性を示した．

  \item[分析規模の拡大 ] 同一並列機械モデルにおける最大実行開始待ち時間最小化問題に対する分枝限定法において，分割生成部分，探索の続行，中断の判定部分の改良により，分析規模を拡大することが可能になった．
...
\begin{description}
  ...
  \item[分析規模の拡大 ] 本研究では，全ての機械モデルにおける最大実行開始待ち時間最小化問題は NP 困難であるという推察のもと，分枝限定法に対して，分割生成時の改良，順列生成時の改良を加えた．
