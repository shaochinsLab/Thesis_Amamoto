...
しかし，$w = 0$ のとき，この問題は JIT スケジューリング問題に対応し，単一機械モデルと同一機械モデルにおいて，多項式時間で判定できることが明らかになっている Cepek and Sung [2004] \cite{JIT}．
...
\begin{quote}
  \begin{description}
    ...
    \begin{description}
      \item[Step 1.] $|\mathcal{J}'| \le |\mathcal{M}|$ を満たすとき，
      \begin{description}
        \item[Step 1.1.] 各 $J \in \mathcal{J} \setminus \mathcal{J}'$ を任意の機械 $M \in \mathcal{M}$ に割り当てる．また，各 $J' \in \mathcal{J}'$ を $M \in \mathcal{M}$ に割り当てる．ただし，$\forall M,M' \in \mathcal{M}\big[S_M \cap S_{M'} = \emptyset\big]$ を満たす．
      \end{description}
      \item[Step 2.] $|\mathcal{J}'| \le |\mathcal{M}|$ を満たさないとき，
      \begin{description}
        \item[Step 2.1.]
        ...
        \begin{description}
          \item[Step 2.3.1.] 最小完了時刻を持つ機械集合の要素 $M_a \in \left\{ \displaystyle \argmin_{M \in \mathcal{M}}C(\mathcal{J}_{S_M})\right\}$ を 1 つ求める．
...
Step 1. では，各ジョブにおける処理開始可能時刻と処理開始可能時刻 + 処理時間 の区間が重なっているジョブの集合の要素数が機械数より少ない時，$\mathcal{J}'$ の各ジョブを別の機械に割り当てることで，実行開始待ち時間が 0 のスケジュールを作ることができる．
それ以外のとき，処理開始可能時刻が最も早いジョブから順に，処理をしていない機械，または処理が一番早く終わった機械に割り当てる．
Step 3. で，処理をしていない機械，または処理が一番早く終わった機械 を取得し $M_a$ と定義している．その後，機械 $M_a$におけるスケジュール $S_{M_a}$ にジョブ $J_i$ を追加する．

この判定を追加したことで，インスタンスにおける最適解から得られる最大実行開始待ち時間が 0
の場合，上記 {\sc Heuristic} でも，最大実行開始待ち時間が 0 となるスケジュールを作ることができる．
...
同一並列機械モデル，無関連並列機械モデルにおける最大実行開始待ち時間最小化問題に対して，本研究で提案したヒューリスティックの実験的評価を行った．評価方法は，各インスタンスに対して，本研究で提案するアルゴリズムにより出力された解から得られた最大実行開始待ち時間 $W_h$ と，厳密解法により出力された最適解から得られた最大実行開始待ち時間 $W_{opt}$ を比較するものである．また，$W_h/W_{opt}$ を {\sc competitive ratio} と表記する．
本研究では，ヒューリスティックと厳密解法に対して以下を評価の軸として実験を行った．

\begin{description}
  \item[ヒューリスティックの評価] ~
  \begin{description}
    \item[軸 1 :] 各インスタンスにおけるジョブの処理時間の幅と {\sc competitive ratio} の関係
    \item[軸 2 :] 各インスタンスにおけるジョブの処理時間の幅と計算時間の関係
    \item[軸 3 :] 同一並列機械モデルと無関連並列機械モデルの {\sc competitive ratio} の比較
...
実験結果の紹介に先立ち，得られた {\sc competitive ratio} の値がどの程度であれば，アルゴリズムの質が良いと言えるかという点について紹介する．
\begin{table}[htb]
  \begin{center}
...
\begin{description}
  \item[最適なスケジュール ] $S_1 : (J_1, J_3, J_5)$，$S_2 : (J_2, J_4, J_6)$．このとき，$w(\mathcal{S}) = 0$ となる．ただし，$\mathcal{S} = S_1 \cup S_2$．
  \item[最適でないスケジュール] $S_1 : (J_1, J_3, J_6)$，$S_2 : (J_2, J_4, J_5)$．このとき，$w(\mathcal{S}) = 3$ となる．ただし，$\mathcal{S} = S_1 \cup S_2$．
\end{description}

上記のとき，{\sc competitive ratio} = {\sc infinity} となり，最適でないスケジュールを生成したアルゴリズムの質は無限大となる．
以下の実験結果は，{\sc competitive ratio} = {\sc infinity} となりうる状況下で得られたものである．つまり，{\sc competitive ratio} = 定数 のとき，アルゴリズムの質は悪くないと言える．
...
\begin{description}
  ...
  最大実行開始待ち時間最小化問題の部分問題である，JIT ジョブ荷重和最大化問題における {\sc 3-SAT} からの還元手法に基づき，最大実行開始待ち時間最小化問題の NP  完全性を示した．
