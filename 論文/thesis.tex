...
\begin{description}
  \item[] 3 - SATISFIABILITY (3 - SAT) の定義は以下の通り．
  \item[Instance : ] ブール型の変数集合 $X = \{x_1,\ldots,x_n\}$ と，$X$ 上の 3 つのリテラルからなる集合 $h$ の集合 $H$．ただし，$H \subseteq 2^{L_X}$ s.t. $\forall h \in H \big[|h| = 3\big]$ を満たす．
...
3 - SAT と 3 - PARTITION の計算複雑さは NP 完全であることが Garey and Johnson [1990] \cite{3SAT} により証明されている．
...
本研究では，3 - SAT からの多項式時間還元により 無関連並列機械モデルにおいて，機械数が入力の一部の場合，最大実行開始待ち時間最小化問題が NP 完全であることを示す．
つまり，3 - SAT のインスタンスから 最大実行開始待ち時間最小化問題 のインスタンスが
多項式時間で構成可能であり，加えて， 3 - SAT  の判定結果が yes ならば
...
\begin{itemize}
  ...
  最大実行開始待ち時間最小化問題（決定問題）は，ジョブが処理開始可能時刻と（処理開始可能時刻 + $w$）の間で処理可能かという問題に置き換えることができる．さらに，最大遅れ時間最小化問題を最大遅れ時間が $\ell$ 以下となるスケジュールが存在するか，という決定問題に変換したとき，ジョブは処理開始可能時刻と（納期 - 処理時間 - $\ell$）の間で処理可能かという問題に置き換えることができる．このとき，どちらの問題もある区間の間で処理可能かという問題に置き換えることができる．
...
式で最適解が求まることが証明されている．つまり，処理開始可能時刻付き最大遅れ時間最小化問題の部分問題に対して，多項式アルゴリズムが存在する．処理開始可能時刻と処理時間，納期の制約を制限することで，多項式で最適解が求まる．部分問題に対する解法は以下の通り．
...
が明らかでない．本研究では，3-SAT からの還元によって，無関連並列機械モデルにおける最大実行開始待ち時間最小化問題（決定問題）が NP 完全であることを示した．
...
Step 3. では，最小の完了時刻を持つ機械を取得し，$M_a$ と定義する．
...
\begin{quote}
  \begin{description}
    ...
    \begin{description}
      ...
      \item[Step 2.] $I = (n,m)$ を入力として，{\sc increment} を実行し，出力が $true$ のとき，以下の処理を繰り返す．
      \begin{description}
        \item[Step 2.1.] $I = (b)$ を入力として，{\sc sort} を実行する．
        ...
        \begin{description}
          ...
          \begin{description}
            \item[Step 2.2.2.1.] $order[i] = j$ のとき，$\mathcal{J}' :=\mathcal{J}' \cup J_i$ とする．
          \end{description}
          \item[Step 2.2.3.] $I = (\mathcal{J}',s,r,p,w,C,0)$ を入力として，\\{allPermutation} を実行する．
        \end{description}
      \end{description}
...
{\sc normalize} では，{\sc increment} で生成した $b$ の各要素が $m$ 未満のとき，割り当てていない機械が存在するので，その分割 $b$ を $m$ となる要素を持つ $b$ に変換している．
...
{\sc increment} について，アルゴリズムの流れを記述する．{\sc increment} では，辞書式順で分割を生成している．配列 $b$ は分割を表しており，$i$ 番目の要素 $b[i]$ は，ジョブ $J_i$ が機械 $b[i]$ に割り当てられていることを表す．ここで，分割の要素数は $\displaystyle \max_{1 \le i \le n}\{b[i]\}$ で表すことができる．
...
\begin{quote}
  \begin{description}
    ...
    \begin{description}
      \item[Step 1.] 要素数 $m$ の配列 $cost$ を定義する．
      ...
      \begin{description}
        ...
        \item[Step 2.4.] {\sc evaluation} により出力されたスケジュールを $S'_{M_j}$ とし，\\$cost[j] := w(S'_{M_j})$ とする．
      \end{description}
      \item[Step 3.] $cost$ を降順でソートする．
      \item[Step 4.] 各 $1 \le j \le m$ における $cost$ の各要素 $cost[j]$ に対応する機械 $M_j$ を $cost$ と同様の順番で並び替えた順列を $order$ に格納する．
      \item[Step 5.] $order$ を出力する．
    \end{description}
  \end{description}
\end{quote}
...
{\sc evaluation} について，アルゴリズムの流れを記述する．
{\sc evaluation} では，機械に割り当てられていないジョブの集合 $\mathcal{J}'$ について，部分問題に対する多項式アルゴリズムを用いて，解を求めている．
...
% allPermutation の説明
{\sc allPermutation} について，アルゴリズムの流れを記述する．
{\sc allPermutation} では，再帰的に順列の生成を行っている．
